...
// Menu BGM state — simple, no async/delayed teardown
...
let prevLbTop1 = null; // track #1 score for new-score notification

// ── ONLINE PRESENCE ──
function initOnlinePresence(playerName){
  try{
    const db = window.firebaseDB;
    const onlineRef = window.firebaseRef(db, 'online');
    const connRef = window.firebaseRef(db, '.info/connected');

    window.firebaseOnValue(connRef, (snap) => {
      if(!snap.val()) return;
      // push my presence
      const me = window.firebasePush(onlineRef, {
        name: playerName || 'Pemain',
        ts: Date.now()
      });
      myOnlineKey = me.key;
      // remove on disconnect
      window.firebaseOnDisconnect(me).remove();
    });

    // listen online count changes
    window.firebaseOnValue(onlineRef, (snap) => {
      const data = snap.val();
      const count = data ? Object.keys(data).length : 0;
      updateOnlineDisplay(count, data);
    });
  } catch(e){ console.warn('Online presence error:', e); }
}

function updateOnlineDisplay(count, data){
...
// ── LEADERBOARD LIVE ──
function initFirebaseLB(){
  try{
    const db = window.firebaseDB;
    const lbRef = window.firebaseRef(db, 'leaderboard');
    const lbQuery = window.firebaseQuery(lbRef, window.firebaseOrderByChild('score'), window.firebaseLimitToLast(10));

    window.firebaseOnValue(lbQuery, (snapshot) => {
      const data = snapshot.val();
      const newCache = data ? Object.values(data).sort((a,b)=>b.score-a.score) : [];

      // Notifikasi skor baru masuk dari orang lain
      if(prevLbTop1 !== null && newCache.length){
        const newTop = newCache[0];
        if(newTop.score > prevLbTop1){
          // cek bukan skor kita sendiri
          const myName = document.getElementById('playerName')?.value?.trim();
          if(newTop.name !== myName){
            showScoreNotif(newTop.name, newTop.score, newTop.diff);
          }
        }
      }
      if(newCache.length) prevLbTop1 = newCache[0].score;

      lbCache = newCache;
      ['menuLbRows','lbRows'].forEach(id=>{
        const el = document.getElementById(id);
        if(el) renderLB(id, null);
      });
    });
  } catch(e){ console.warn('Firebase LB init failed:', e); }
}

// Notifikasi skor baru dari pemain lain
function showScoreNotif(name, pts, diff){
  const n = document.createElement('div');
  n.className = 'score-notif';
  n.innerHTML = `🏆 <b>${name}</b> baru saja submit <b>${pts}</b> pts [${diff||'?'}]!`;
  document.body.appendChild(n);
  setTimeout(()=>n.remove(), 4000);
}

async function addLBScore(name, pts){
  try{
    const db = window.firebaseDB;
    await window.firebasePush(window.firebaseRef(db,'leaderboard'), {
    ...
    console.warn('Firebase write failed, fallback localStorage:', e);
    const lb = JSON.parse(localStorage.getItem('waz_lb')||'[]');
    lb.push({name:name.trim()||'Anonymous',score:pts,diff:selectedDiff});
    lb.sort((a,b)=>b.score-a.score);
    localStorage.setItem('waz_lb', JSON.stringify(lb.slice(0,10)));
  }
}

function renderLB(cid, highlight=null){
  const el = document.getElementById(cid);
  if(!el) return;
  const lb = lbCache.length ? lbCache : JSON.parse(localStorage.getItem('waz_lb')||'[]');
  if(!lb.length){
    el.innerHTML='<div style="color:#2a2a2a;font-size:12px;text-align:center">Belum ada skor...</div>';
    return;
  }
  ...
  el.innerHTML = lb.slice(0,10).map((e,i)=>`
    <div class="lb-row">
      ...
      <span class="lb-name">${e.name} <small style="color:#3a3a3a;font-size:10px">[${e.diff||'?'}]</small></span>
      <span class="lb-pts">${e.score}${highlight!==null&&e.score===highlight&&i===lb.findIndex(x=>x.score===highlight)?'<span class="lb-new">NEW</span>':''}</span>
    </div>`).join('');
}
...
function startGame(){
  // Paksa clear semua timer lama dulu
  gameActive=false;
  clearInterval(countdownTimer); countdownTimer=null;
  clearTimeout(spawnTimer); spawnTimer=null;
  stopPuBarRefresh();
  for(let i=0;i<HOLES;i++){ clearTimeout(holeTimers[i]); holeTimers[i]=null; }
  if(activePowerups.slow) clearTimeout(activePowerups.slow.timerId);
  if(activePowerups.shield) clearTimeout(activePowerups.shield.timerId);
  ...
  countdownTimer=setInterval(()=>{
    if(!gameActive){clearInterval(countdownTimer);countdownTimer=null;return;}
    timeLeft--;
...
function endGame(){
  ...
  stopBGM();
  document.getElementById('vuMeter').classList.add('muted');
  if(soundEnabled){ setTimeout(()=>{ startMenuBGM(); }, 900); }
  SFX.gameOver();

  if(raceMode){ endRaceGame(); return; }
  ...
  if(leveled){
    setTimeout(()=>{
      showLvUpBanner(newLv);
      SKINS.forEach(sk=>{
        if(sk.unlockLv>prevLv&&sk.unlockLv<=newLv)
          setTimeout(()=>showNotif(`🎨 Skin "${sk.name}" ${sk.emoji} terbuka!`),1800);
      });
    },600);
  }

  renderLB('lbRows',null);
  document.getElementById('playerName').value=localStorage.getItem('waz_lastPlayerName')||'';
...
// Single entry point: any click anywhere starts menu BGM once
document.body.addEventListener('click',()=>{
  if(!menuRunning&&soundEnabled){
...
document.getElementById('saveScoreBtn').addEventListener('click', async ()=>{
  ...
  if(name.trim()) localStorage.setItem('waz_lastPlayerName', name.trim());
  await addLBScore(name, score);
  renderLB('lbRows', score);
...
let raceMode = false;
let raceRoomCode = null;
let raceIsHost = false;
let raceMyName = '';
let raceScoreInterval = null;
let raceRoomRef = null;
let raceUnsubPlayers = null;
let raceUnsubState = null;
...
// ── Create Room ──
function createRace(){
  raceMyName = document.getElementById('raceNameInput').value.trim() || 'Host';
  raceRoomCode = randRoomCode();
  raceIsHost = true;
  joinRaceRoom(raceRoomCode, raceMyName, true);
}

// ── Join Room ──
function joinRace(){
  raceMyName = document.getElementById('raceNameInput').value.trim() || 'Pemain';
  const code = document.getElementById('joinRoomInput').value.trim().toUpperCase();
  if(!code || code.length < 4){ showNotif('❌ Masukkan kode room!'); return; }
  raceRoomCode = code;
  raceIsHost = false;
  joinRaceRoom(code, raceMyName, false);
}

function joinRaceRoom(code, name, isHost){
  if(!window.firebaseDB){ showNotif('❌ Firebase belum siap!'); return; }
  try{
    const db = window.firebaseDB;
    raceRoomRef = window.firebaseRef(db, `races/${code}`);
    const myRef = window.firebaseRef(db, `races/${code}/players/${name}`);

    // Add myself to room
    window.firebasePush && null; // no-op
    const {set: fbSet} = window.firebaseSetFn ? {set: window.firebaseSetFn} : {};

    window.firebaseSetVal(myRef, {
      name, score: 0, isHost, ready: true, ts: Date.now()
    });
    window.firebaseOnDisconnect(myRef).remove();

    // Show lobby
    document.getElementById('raceSetupCard').style.display = 'none';
    document.getElementById('raceLobby').style.display = 'flex';
    document.getElementById('raceCodeDisplay').textContent = code;
    if(isHost){
      document.getElementById('startRaceBtn').style.display = 'inline-block';
      document.getElementById('raceWaiting').style.display = 'none';
    } else {
      document.getElementById('startRaceBtn').style.display = 'none';
      document.getElementById('raceWaiting').style.display = 'block';
    }

    // Listen players list
    const playersRef = window.firebaseRef(db, `races/${code}/players`);
    raceUnsubPlayers = window.firebaseOnValue(playersRef, (snap)=>{
      const data = snap.val();
      renderRaceLobbyPlayers(data);
    });

    // Listen game state (host starts → all redirect to game)
    const stateRef = window.firebaseRef(db, `races/${code}/state`);
    raceUnsubState = window.firebaseOnValue(stateRef, (snap)=>{
      const state = snap.val();
      // Hanya listen 'playing' untuk mulai game
      // 'finished' tidak perlu — semua pemain langsung tampil hasil sendiri
      if(state === 'playing' && !gameActive){
        startRaceGame();
      }
    });

  } catch(e){ console.error('Race join error:', e); showNotif('❌ Gagal join room!'); }
}

function renderRaceLobbyPlayers(data){
  const el = document.getElementById('racePlayersLobby');
  if(!el || !data) return;
  el.innerHTML = Object.values(data).map(p=>`
    <div class="race-player-row">
      <div class="race-player-dot"></div>
      <div class="race-player-name">${p.name}</div>
      ${p.isHost ? '<span class="race-player-host">HOST</span>' : ''}
    </div>`).join('');
}

// ── Start Race (host only) ──
function startRaceFromLobby(){
  if(!raceIsHost || !raceRoomCode) return;
  const db = window.firebaseDB;
  window.firebaseSetVal(window.firebaseRef(db, `races/${raceRoomCode}/state`), 'playing');
}

function startRaceGame(){
  raceMode = true;
  document.getElementById('raceHud').style.display = 'flex';
  startGame(); // reuse existing startGame
  // Push my score to Firebase every second
  raceScoreInterval = setInterval(()=>{
    if(!raceRoomCode || !raceMyName || !window.firebaseDB) return;
    window.firebaseSetVal(
      window.firebaseRef(window.firebaseDB, `races/${raceRoomCode}/players/${raceMyName}/score`),
      score
    );
  }, 1000);

  // Listen all players scores for live HUD
  const playersRef = window.firebaseRef(window.firebaseDB, `races/${raceRoomCode}/players`);
  window.firebaseOnValue(playersRef, (snap)=>{
    const data = snap.val();
    if(data) renderRaceHud(data);
  });
}

function renderRaceHud(data){
  const el = document.getElementById('racePlayers');
  if(!el) return;
  const players = Object.values(data).sort((a,b)=>b.score-a.score);
  const topScore = players[0]?.score || 0;
  el.innerHTML = players.map(p=>{
    const isMe = p.name === raceMyName;
    const isLeading = p.score === topScore && topScore > 0;
    return `<div class="race-player-chip ${isMe?'me':''} ${isLeading?'leading':''}">
      ${isLeading ? '<span class="rpc-crown">👑</span>' : ''}
      <span class="rpc-name ${isMe?'me':''} ${isLeading?'leading':''}">${p.name}</span>
      <span class="rpc-score">${p.score}</span>
    </div>`;
  }).join('');
}

// Called when game ends during race mode
function endRaceGame(){
  if(!raceMode) return;
  raceMode=false;
  clearInterval(raceScoreInterval);

  // Push final score ke Firebase (fire and forget)
  if(raceRoomCode && raceMyName && window.firebaseDB){
    try{
      window.firebaseSetVal(
        window.firebaseRef(window.firebaseDB, `races/${raceRoomCode}/players/${raceMyName}/score`), score
      );
      window.firebaseSetVal(
        window.firebaseRef(window.firebaseDB, `races/${raceRoomCode}/players/${raceMyName}/done`), true
      );
      if(raceIsHost){
        setTimeout(()=>{
          window.firebaseSetVal(window.firebaseRef(window.firebaseDB, `races/${raceRoomCode}/state`), 'finished');
        }, 800);
      }
    }catch(e){ console.warn('Race Firebase error:', e); }
  }

  // Semua pemain (host & non-host) langsung tampil hasil
  // tanpa nunggu state Firebase
  document.getElementById('raceHud').style.display='none';
  setTimeout(()=>showRaceResults(raceRoomCode), 500);
}

function showRaceResults(code){
  // Tampilkan screen dulu
  document.getElementById('raceResult').style.display='flex';
  document.getElementById('raceLobby').style.display='none';
  document.getElementById('raceSetupCard').style.display='none';
  showScreen('raceScreen');

  if(!code || !window.firebaseDB){
    // Fallback: tampilkan skor sendiri saja
    document.getElementById('raceResultRows').innerHTML=`
      <div class="race-result-row winner">
        <span class="race-result-rank gold">1</span>
        <span class="race-result-name">${raceMyName} (kamu)</span>
        <span class="race-result-pts">${score}</span>
      </div>`;
    return;
  }
  ...
  // Timeout 3 detik — kalau Firebase lambat, pakai data yang ada
  const timeout = setTimeout(()=>{
    document.getElementById('raceResultRows').innerHTML=`
      ...
      <div style="color:#444;font-size:11px;text-align:center;margin-top:8px;">⚠ Menunggu data temen...</div>`;
  }, 3000);
  ...
  window.firebaseOnValue(playersRef, (snap)=>{
    ...
    const players = Object.values(data).sort((a,b)=>b.score-a.score);
    const medals = ['gold','silver','bronze'];
    document.getElementById('raceResultRows').innerHTML = players.map((p,i)=>`
      <div class="race-result-row ${i===0?'winner':''}">
        <span class="race-result-rank ${medals[i]||''}">${i+1}</span>
        <span class="race-result-name">${p.name}${p.name===raceMyName?' (kamu)':''}</span>
        <span class="race-result-pts">${p.score}</span>
      </div>`).join('');
  }, {onlyOnce: true});
}
...
function cleanupRace(){
  clearInterval(raceScoreInterval);
  if(raceUnsubPlayers) raceUnsubPlayers();
  if(raceUnsubState) raceUnsubState();
  raceMode = false; raceRoomCode = null; raceIsHost = false;
  document.getElementById('raceHud').style.display = 'none';
  document.getElementById('raceSetupCard').style.display = 'flex';
  document.getElementById('raceLobby').style.display = 'none';
  document.getElementById('raceResult').style.display = 'none';
}

// ── Race Events ──
document.getElementById('raceBtn').addEventListener('click',()=>{ showScreen('raceScreen'); });
document.getElementById('raceBackBtn').addEventListener('click',()=>{ cleanupRace(); showScreen('menu'); });
document.getElementById('createRoomBtn').addEventListener('click', createRace);
document.getElementById('joinRoomBtn').addEventListener('click', joinRace);
document.getElementById('startRaceBtn').addEventListener('click', startRaceFromLobby);
document.getElementById('racePlayAgainBtn').addEventListener('click',()=>{
  cleanupRace();
  showScreen('raceScreen');
  document.getElementById('raceSetupCard').style.display = 'flex';
});
document.getElementById('raceBackMenuBtn').addEventListener('click',()=>{ cleanupRace(); showScreen('menu'); });
...
const waitFirebase = setInterval(()=>{
  if(window.firebaseDB){
    ...
    initFirebaseLB();
    initOnlinePresence(localStorage.getItem('waz_lastPlayerName')||'Pemain');
  }
},200);